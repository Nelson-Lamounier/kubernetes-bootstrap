        ...
        inputs: {
          ...
          Parameters: {
            commands: [
              ...
              `set -euo pipefail`,
              ``,
              `# AMI-baked baseline is at /opt/k8s-bootstrap/. Sync the boot/steps tree`,
              `# from S3 so that script changes take effect without an AMI re-bake.`,
              `STEPS_DIR="/opt/k8s-bootstrap/${step.scriptPath.replace(/\/[^/]+$/, "")}"`,
              `SCRIPT="/opt/k8s-bootstrap/${step.scriptPath}"`,
              ``,
              `aws s3 sync "s3://{{ S3Bucket }}/k8s-bootstrap/${step.scriptPath.replace(/\/[^/]+$/, "")}/" "$STEPS_DIR/" --region "{{ Region }}" --quiet`,
              ``,
