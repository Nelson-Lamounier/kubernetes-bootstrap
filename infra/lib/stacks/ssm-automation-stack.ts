        ...
        const bootstrapRunner = new SsmRunCommandDocument(this, 'BootstrapRunnerCommand', {
            ...
            steps: [{
                ...
                commands: [
                    ...
                    '',
                    '# Sync the step directory from S3 so committed script fixes take effect',
                    '# without requiring an AMI re-bake. Node modules stay AMI-baked.',
                    'aws s3 sync "s3://{{S3Bucket}}/k8s-bootstrap/$(dirname "$SCRIPT_PATH")/" "$STEPS_DIR/" --region "{{Region}}" --quiet',
                    '',
