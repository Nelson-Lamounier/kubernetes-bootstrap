        ...
        const bootstrapRunner = new SsmRunCommandDocument(this, 'BootstrapRunnerCommand', {
            ...
            steps: [{
                ...
                commands: [
                    ...
                    '',
                    '# Sync the FULL sm-a/ tree from S3 so committed script fixes take',
                    '# effect across both control_plane.ts and the bootstrap_argocd.ts',
                    '# it invokes — without requiring an AMI re-bake. Node modules stay',
                    '# AMI-baked (excluded). Previously this synced only $(dirname SCRIPT_PATH),',
                    '# which left sm-a/argocd/ stale at runtime when control_plane.ts ran.',
                    'aws s3 sync "s3://{{S3Bucket}}/k8s-bootstrap/sm-a/" "/opt/k8s-bootstrap/sm-a/" --region "{{Region}}" --exclude "*/node_modules/*" --quiet',
                    '',
